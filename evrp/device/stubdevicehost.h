...
// 默认桩：Ping 成功；录制路径实现「StartRecording → ReadInputEvents（阻塞至
// StopRecording）」会话模型， 不打开 evdev、不推送真实
...
class StubDeviceHost final : public api::IDeviceHost {
 public:
  ...
  api::ApiResult<void> StartRecording(
  ...
  api::ApiResult<api::CursorAvailability> GetCursorPositionAvailability()
      override;
