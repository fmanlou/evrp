...
Server::Server(const ISetting& deviceSettings) {
  workGuard_.emplace(asio::make_work_guard(ioContext_.get_executor()));
  localListener_ =
      std::make_unique<evrp::device::server::LocalInputListener>();
  inputListener_ = std::make_unique<evrp::device::server::PostedInputListener>(
      *localListener_, ioContext_);
  cursorPosition_ =
      std::make_unique<evrp::device::server::LocalCursorPosition>();
