...
library/
  ...
  asio/         # Standalone Asio：include/（上游）、lib/libasio_separate.a（脚本内编译 third_party/asioimpl）
...
总入口 **`scripts/install-third-party-to-library.sh`** 会依次调用各库独立脚本；约定 **CMake 构建树**为 **`build/third-party/<库名>/`**（Lua、gflags、gRPC、Protobuf、GoogleTest、**Asio**、fmt、spdlog、log 等一致）。**fmt/spdlog** 等 **`<库名>-src/`** 浅克隆在安装成功后删除；**Asio** 上游克隆在 **`third_party/asio/`**（与 **`third_party/log`** 同类，见根目录 **`.gitignore`**），独立编译工程在可提交的 **`third_party/asioimpl/`**。
...
| **`scripts/install-asio-to-library.sh`** | `library/asio/`（**`third_party/asio`** → `include/`；**`third_party/asioimpl`** → `lib/libasio_separate.a`） |
...
（依次安装 Lua、GoogleTest、Protobuf、gRPC、gflags、Asio、log；其中 **log** 步骤会按需安装 **fmt、spdlog**；gRPC **使用已安装到 `library/protobuf` 的 Protobuf**。）若对应 **`library/<组件>/`** 下已有安装结果，会**跳过**该组件。若要**强制全部重装**：
