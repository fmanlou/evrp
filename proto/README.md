...
业务代码通过 **`evrp/device/{api,internal,server,client}/`**（如 `server/server.h`、`client/clientfactory.h`、`IDeviceHost`、`IDeviceClient` 等）访问设备能力，**不** include 本目录生成的 `*.pb.h` 或 gRPC（proto 互转见 `internal/deviceprotoconv.h`）；详见 [`docs/API_LAYER.md`](../docs/API_LAYER.md)。
